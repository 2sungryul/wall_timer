...
void mycallback(rclcpp::Node::SharedPtr node, rclcpp::Publisher<std_msgs::msg::String>::SharedPtr mypub)
{
    static int count;
...
int main(int argc, char* argv[])
{
    ...
    //auto fn = std::bind(mycallback, node, pub);
    auto timer = node->create_wall_timer(100ms, fn);
    //auto timer = node->create_wall_timer(100ms, std::bind(mycallback, node, pub));
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}